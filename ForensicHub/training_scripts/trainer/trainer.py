"""
Local override of IMDLBenCo's train_one_epoch.

This version adds proper GPU memory cleanup when NaN/Inf loss is detected
and the batch is skipped. Without this, the computational graph from the
forward pass remains in GPU memory (since backward() was never called),
causing an OOM on the next forward pass.
"""
import math
import sys
import gc
...
def train_one_epoch(model: torch.nn.Module,
    ...
    if args.if_not_amp:
        ...
        amp_placeholder = torch.amp.autocast("cuda")
    ...
    for data_iter_step, data_dict in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        mask_pred = output_dict.get('pred_mask')
        ...
        # called, so the entire computational graph from the forward pass stays
