...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "ForensicHub benchmark training launch!", add_help=True
    )
    parser.add_argument(
        "--config", type=str, help="Path to YAML config file", required=True
    )
...
def main(
    ...
    if args.start_epoch > 0 and not args.resume:
        ...
        print(
            Fore.YELLOW
            + "  WARNING: start_epoch > 0 but no 'resume' checkpoint provided!"
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
            + "  The model and optimizer will start from scratch, but the LR"
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
            + f"  scheduler will use epoch {args.start_epoch}. This is likely not"
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
            + "  what you want. To properly resume training, set:"
            + Style.RESET_ALL
        )
        print(
            Fore.YELLOW + f"       resume: <path_to_checkpoint.pth>" + Style.RESET_ALL
        )
        print(
            Fore.YELLOW
            + f"  (start_epoch will be set automatically from the checkpoint)"
            + Style.RESET_ALL
        )
    ...
    print(
        Fore.CYAN
        + f"  Tip: To stop training gracefully at the end of an epoch, run:"
        + Style.RESET_ALL
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % args.test_period == 0 or epoch + 1 == args.epochs:
            ...
            if evaluate_metric_value > best_evaluate_metric_value:
                ...
                print(
                    f"Average {' '.join([evaluator.name for evaluator in evaluator_list])} = {evaluate_metric_value}"
                )
        ...
        if check_stop_signal(args.log_dir):
            ...
            print(
                Fore.YELLOW
                + "  STOP signal detected! Saving model and stopping training..."
                + Style.RESET_ALL
            )
            print(Fore.YELLOW + f"{'='*60}\n" + Style.RESET_ALL)
            ...
            print(
                Fore.GREEN
                + f"  To resume training, set in your YAML:"
                + Style.RESET_ALL
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Properly cleanup distributed process group to avoid resource leak warning
